        ...
        NavigationStack {
            ZStack {
                ...
                if store.items.isEmpty {
                    ContentUnavailableView("No Sites Yet", systemImage: "tray", description: Text("Tap + to log your first one."))
                } else {
                    List {
                        ForEach(store.items) { item in
                            HStack {
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    Text("Rating (1-5): " + String(format: "%.1f", item.value2))
            ...
            .navigationTitle("Campground Log")
            .toolbar {
...
struct AddItemView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    TextField("Site Name", text: $titleText)
                        .focused($focusedField, equals: .title)
                        .accessibilityIdentifier("addTitleField")
                    TextField("Rating (1-5)", text: $value2Text)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        let item = Site(title: titleText.isEmpty ? "Site" : titleText, value2: Double(value2Text) ?? 0, notes: notesText)
                        if store.add(item) {
                            dismiss()
                        }
                    }
