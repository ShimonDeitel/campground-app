import XCTest
@testable import CampgroundLog

@MainActor
final class CampgroundLogTests: XCTestCase {
    var store: Store!
    ...
    func testAddItemIncreasesCount() {
        ...
        let added = store.add(Site(title: "Test", value2: 0))
        XCTAssertTrue(added)
    ...
    func testCannotAddBeyondFreeLimitWhenNotPro() {
        ...
        for i in 0..<Store.freeLimit {
            _ = store.add(Site(title: "Item \(i)", value2: 0))
        }
        let result = store.add(Site(title: "Overflow", value2: 0))
    ...
    func testDeleteRemovesItem() {
        let item = Site(title: "ToDelete", value2: 0)
        _ = store.add(item)
    ...
    func testUpdateModifiesItem() {
        var item = Site(title: "Original", value2: 0)
        _ = store.add(item)
        item.title = "Updated"
        store.update(item)
    ...
    func testDeleteAtOffsets() {
        _ = store.add(Site(title: "A", value2: 0))
        _ = store.add(Site(title: "B", value2: 0))
    ...
    func testPersistenceRoundTrip() {
        _ = store.add(Site(title: "Persisted", value2: 0))
        store.save()
        let reloaded = Store()
